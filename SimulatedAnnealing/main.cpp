...
int* SimulatedAnnealing(packing pacote, int n){
    int *solucao, *solucao_1, *best, aux, tmp;
    double e, r,  temperatura = 100;
    solucao = GeradorRandom(pacote);
    best = Cpy(solucao, pacote.tamanho);

    for(int i = 0 ; i < n; i++){
        iter++;
        solucao_1 = VizinhoRandom(solucao, pacote);

        aux = Residuo(solucao, pacote);
        tmp = Residuo(solucao_1, pacote);


        if(tmp <= aux){
            solucao = solucao_1;

            if(tmp <= Residuo(best, pacote))
                best = Cpy(solucao_1, pacote.tamanho);

        }else
            e = (double) exp(((double)(aux-tmp)/(double)temperatura));
            r = (double) rand() / (double) RAND_MAX;

            if(e > r){

                solucao = Cpy(solucao_1, pacote.tamanho);
        }
        if(temperatura > 0.001) temperatura = (double)temperatura * 0.95;
    }
    cout<<"\n"<<"residuo "<<Residuo(best, pacote)<<"\n";
    return best;
}
...
int main(){
    ...
    int *a = SimulatedAnnealing(pacote, 1000);
