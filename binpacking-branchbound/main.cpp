...
void SalvarSolucao(int *vet, int tam){
    FILE *arq;
    arq = fopen ("saida.txt", "a");
    if(arq == NULL){
        cout<<"erro ao salvar solução";
        exit(0);
    }
    for(int i = 0; i < tam; i++){
        fprintf(arq, "%d\r\n", vet[i]);
...
int* Branch_and_Bound(int *vet, int peso_atual, int peso_max,
                              int posicao, int tam, int tamanho, int n_elementos){
    ...
    if(tam > 1){
        if(peso1 <= peso_max){
            solucao_dir = Branch_and_Bound(vet, peso1, peso_max, posicao + 1, tam - 1, tamanho, n_elementos + 1);
        }
        if(peso2 <= peso_max){
            solucao_esq = Branch_and_Bound(vet, peso2, peso_max, posicao + 1, tam - 1, tamanho, n_elementos);
        }
        if((solucao_dir!=NULL) && solucao_esq != NULL){
            int n_elementos_dir, n_elementos_esq;
            n_elementos_dir = contaElementos(solucao_dir, tam) + 1 + n_elementos;
            n_elementos_esq = contaElementos(solucao_esq, tam) + n_elementos;

            if((solucao_dir[0] == solucao_esq[0]) && (n_elementos_dir < n_elementos_esq)){

                solucao_dir = pop(solucao_dir, 1, tam + 1);
                return solucao_dir;

            }else if ((solucao_dir[0] == solucao_esq[0])&& (n_elementos_dir > n_elementos_esq)){
...
int main(){
    ...
    while (pacote.flag != 0){
        solucao = Branch_and_Bound(pacote.vet, 0, pacote.peso_max, 0, pacote.tamanho, pacote.tamanho, 0);
        //print(solucao, pacote.tamanho + 1);

        if (solucao != NULL) pacote = Atualizacao(pacote, solucao);
        n_mochilas ++;
        SalvarSolucao(solucao, pacote.tamanho + 1);
    ...
    SalvarTexto("O numero minimo de pacotes é:\n");
    char str[100];
